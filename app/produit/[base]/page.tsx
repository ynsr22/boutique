...
const OrderSummary = memo(({ quantity, selectedMaterial, selectedAccessories, totalPrice }: OrderSummaryProps) => (
  <div className="bg-gray-50 border border-gray-200 rounded-md p-4 shadow-sm">
    <h3 className="text-md font-semibold mb-2 text-gray-800">Résumé de la commande</h3>
    <div className="text-sm text-gray-700 space-y-1">
      <p>
        <span className="font-medium">Base :</span> {BASE_PRICE.toLocaleString()}€ x {quantity}
      </p>
      <p>
...
const BasePage = () => {
  ...
  const totalPrice = useMemo(() => {
    const accessoriesTotal = Object.entries(selectedAccessories).reduce((sum, [categoryKey, accessoryId]) => {
      const accessory = ACCESSORY_CATEGORIES[categoryKey]?.items.find((item) => item.id === accessoryId);
  ...
  const handleAddToCart = useCallback(() => {
    ...
    const newItem = {
      ...
      quantity,
      accessories: Object.entries(selectedAccessories).map(([category, id]) => ({
        category,
