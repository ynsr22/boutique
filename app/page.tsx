...
export default function ProductsPage() {
    ...
    async function fetchProducts() {
      try {
        ...
        const normalizedData = data.map(item => ({
          ...item,
          prix: typeof item.prix === 'string' ? parseFloat(item.prix) : item.prix,
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    const filtered = items.filter(item => {
      // Filtre par prix
      const matchesPrice = item.prix >= priceRange[0] && item.prix <= priceRange[1];
      ...
      return matchesPrice && matchesSearch && matchesDepartment && matchesMaterial;
    });
  ...
  }, [items, priceRange, searchQuery, selectedDepartments, selectedMaterials, sortOption]);
              ...
              {filteredAndSortedItems.length > 0 && !loading && (
                <div className="mt-4 p-3 bg-yellow-50 rounded border border-yellow-100">
                  <p className="text-sm text-yellow-700">
                    {filteredAndSortedItems.length} {filteredAndSortedItems.length === 1 ? 'produit trouvé' : 'produits trouvés'}
                  </p>
                </div>
              )}
          ...
          <section className="md:col-span-3">
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-lg p-8 text-center border border-gray-200 shadow-sm">
                ...
                <p className="text-gray-600 mb-4">Contacter Yanis pour des moyens spécifiques.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
